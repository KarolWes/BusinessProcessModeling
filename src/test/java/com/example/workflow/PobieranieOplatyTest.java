...
import org.camunda.bpm.engine.test.junit5.*;
...
public class PobieranieOplatyTest {
    @Rule
    ...
    public void testPobranieOplaty() {
        ...
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("PobranieOplaty", variablesIn);
        assertEquals(processInstance.isEnded(), true);
        List<HistoricVariableInstance> oplata_nrTransakcjiList =
                historyService.createHistoricVariableInstanceQuery().variableName("oplata_nrTransakcji").list();
                        HistoricVariableInstance oplata_nrTransakcji =
                                oplata_nrTransakcjiList.get(oplata_nrTransakcjiList.size()-1);
        assertEquals("ABC321", oplata_nrTransakcji.getValue());
    }

    @Test
    @Deployment(resources = {"PobranieOplaty.bpmn", "OcenaPodania.dmn",
            "process.bpmn"})
    public void testZaliczenieSemestru() throws InterruptedException { RuntimeService
        ...
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("ZaliczenieSemestru", variablesIn);
        ...
        complete(task(processInstance), withVariables("oplata_kwota", -11,
                "oplata_czyZgoda", true));
        assertThat(processInstance).isWaitingAt("WprowadzenieDanychPlatnosci");
        assertThat(processInstance).variables().containsEntry("oplata_status",
                "OplataKwotaErrCode");
        complete(task(processInstance), withVariables("oplata_kwota", 100,
                "oplata_czyZgoda", true));
        assertThat(processInstance).isWaitingAt("OdbiorDecyzji");
        assertThat(processInstance).variables().containsEntry("decyzja_czyPozytywna",
                true);
        assertThat(processInstance).variables().containsEntry("oplata_kwota", 100);
        assertThat(processInstance).variables().containsEntry("oplata_nrTransakcji",
                "ABC321");
    ...
    @Test
    @Deployment(resources = {"OcenaPodania.dmn", "process.bpmn",
            "Odwolanie.bpmn"})
    public void testZaliczenieSemestruZodwolaniem() throws InterruptedException {
        ...
        VariableMap variablesIn = Variables.createVariables()
                ...
                .putValue("podanie_uzasadnienie", "krótkie");
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("ZaliczenieSemestru",
